...
const Dashboard = (props) => {
  ...
  let symbol = document.location.pathname.split('/').slice(2, 3).toString();
  if (!symbol) {
  ...
  const headCells = [
    ...
    { id: 'adjClose', numeric: true, disablePadding: false, label: 'Adj Close**' },
    { id: 'perChange', numeric: true, disablePadding: false, label: 'Price Change' },
    { id: 'volume', numeric: true, disablePadding: false, label: 'Volume' },
  ];

  const followHandler = async (value) => {
    if (isAuthenticated) {
  ...
  const unFollowHandler = async (value) => {
    if (isAuthenticated) {
      ...
      <Container style={{ maxWidth: 1400 }}>
        <Grid style={{ margin: '2% 0', }}>
          <Typography variant="h4" className={classes.Headlink} gutterBottom>
            {price.symbol && price.longName && (`${price.symbol} - ${price.longName}`)}
          </Typography>
