...
const ChartBox = (props) => {
  ...
  const historicalData = []; const volume = []; const dataLength = historical.length;
  let i = 0;
  ...
  return (
    <Card className={clsx(classes.root, className)} style={{ border: '1px solid #ccc' }}>
      <CardHeader
        title={<Title {...props} />}
        classes={{
          title: classes.title,
...
const Title = (props) => {
  ...
  const isFollows = follows.some((x) => x === symbol);
  const follow = () => {
    onFollow(symbol);
  };
  const unFollow = () => {
